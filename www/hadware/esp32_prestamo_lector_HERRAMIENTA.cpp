// SMARTLABS · ESP32 lector de HERRAMIENTA en estación de préstamo (SMART10002)
// =========================================================================
// Recibe el UID de la pegatina NTAG213 de la herramienta por UART (Arduino+
// RC522 con arduino_lector_universal.ino), lo publica a {SN}/loan_querye y
// reacciona a los comandos del backend (prestado / devuelto / nofound /
// nologin).
//
// Mejoras respecto a la versión legacy (main_equiposL.cpp):
//   - Credenciales WiFi/MQTT en secrets.h (gitignored).
//   - char[] en lugar de Arduino String para el UID.
//   - QueueHandle_t entre core 0 (UART) y core 1 (MQTT).
//   - WiFi.begin con timeout y reset automático.
//   - MQTT con Last Will Testament: {SN}/status retained.
//   - Reconexión MQTT no bloqueante.
//   - Sin temprature_sens_read.
...
// -------------------------------------------------------------------------
static const char*  DEVICE_SN   = "SMART10002";
static const char*  TOPIC_QUERY = "loan_querye";  // → publica el UID aquí
static const IPAddress LOCAL_IP(192, 168, 0, 33);
...
static void onMqttMessage(char* topic, byte* payload, unsigned int length) {
  ...
  if (t == topicOf("user_name")) {
    // En esta estación el backend usa este topic para enviar el nombre
    // de la herramienta (no del usuario).
...
static void screenNotFound() {
  ...
  display.setTextColor(SH110X_WHITE);
  display.setCursor(0, 0);
  display.println("Equipo no encontrado");
...
static void screenNoLogin() {
  ...
  display.setTextColor(SH110X_WHITE);
  display.setCursor(0, 0);
  display.println("Inicia sesion primero");
  display.println("(coloca tu credencial");
  display.println("en el otro lector).");
  display.display();
  delay(4000);
  screenIdle();
}
