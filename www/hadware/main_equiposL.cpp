...
const String serial_number = "SMART10002";
    ...
    for (;;)
    {
        ...
        while (millis() - startTime < TIMEOUT)
        {
            if (Serial2.available())
            {
                ...
                if (char_now == STOP_CHAR || rfid.length() >= MAX_RFID_LENGTH)
                {
                    break;
                }
        ...
        if (rfid.length() == MAX_RFID_LENGTH)
        {
...
void loop()
{
    ...
    if (send_access_query == true)
    {
        ...
        char topic[25];
        String topic_aux = serial_number + "/loan_querye";
        topic_aux.toCharArray(topic, 25);
...
void noLoadAcount()
{
    ...
    display.println("");
    display.println("EQUIPO NO ENCONTRADO INTENTA DE NUEVO");
    display.display();
    delay(4500);
    iddle();
}
...
void nologin()
{
    ...
    display.setTextColor(SH110X_WHITE);
    display.setCursor(0, 0);
    display.println("INICIA SESION");
    display.println("E INTENTA DE NUEVO");
    display.display();
    delay(4500);
    iddle();
}
...
void iddle()
{
    ...
    display.setCursor(0, 0);
    display.println("COLOCA EL EQUIPO ");
    display.display();
}
