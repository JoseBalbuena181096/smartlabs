...
if( isset($_POST['name']) && isset($_POST['brand']) && isset($_POST['rfid'])) {
  ...
  $rfid_ = strip_tags($_POST['rfid']);
    $result = $conn->query("SELECT * FROM `equipments` WHERE `equipments_rfid` = '".$rfid_."' ");
    $equipments_ = $result->fetch_all(MYSQLI_ASSOC);
    //cuento cuantos elementos tiene $tabla,
    $count = count($equipments_);
    //solo si no hay un usuario con mismo mail, procedemos a insertar fila con nuevo usuario
    if ($count == 0){
      $conn->query("INSERT INTO `equipments` (`equipments_name`, `equipments_rfid`, `equipments_brand`) VALUES ('".$name_."', '".$rfid_."', '".$brand_."');");
      $msg.="equipo creado  <br>";
    }else{
      $msg.="equipo existente <br>";
    }
    $result_ = $conn->query("SELECT * FROM `equipments` ORDER BY `equipments_id` DESC");
    $equipments = $result_->fetch_all(MYSQLI_ASSOC);
}else{
            ...
            <div class="col-sm-12">
                  <div class="box">
                    ...
                    <form method="post" target="register.php" name="form">
                        ...
                        <div class="md-form-group" style="margin-left:25px">
                            <input name="brand" type="text" class="md-input" value="" required>
                            <label>MARCA: </label>
                        </div>
                        <div class="md-form-group" style="margin-left:25px">
                  ...
                  <table class="table table-striped b-t">
                    ...
                    <tbody>
                      <?php foreach ($equipments as $equipments) { ?>
                        <tr>
                          <td><?php echo $equipments['equipments_name']; ?></td>
                          <td><?php echo $equipments['equipments_brand']; ?></td>
                          <td><?php echo $equipments['equipments_rfid']; ?></td>
                        </tr>
                      <?php } ?>
                    </tbody>    
                  </table>
...
function process_msg(topic, message){
  ...
  var device_serie = $( "#device_id" ).val();
  if ((query == "loan_querye"  || query == "scholar_query" ) && device_serie === serial_number ){
...
client.on('connect', () => {
    ...
    <?php foreach ($devices as $device) { ?>      
      client.subscribe('<?php echo $device['devices_serie'] ?>/loan_querye', { qos: 0 }, (error) => {})
