      ...
      foreach ($filtered_loans as $loan) {
        if($loan["loans_state"]=='1'){
          ...
          if ($check_result->num_rows == 0) {
            ...
            $output .= '<td><button class="btn btn-sm btn-info return-item" data-user-rfid="' . $user_card_number . '" data-equipment-rfid="' . $loan["equipments_rfid"] . '" data-equipment-name="' . $loan["equipments_name"] . '">Devolver</button></td>';
            $output .= '</tr>';
...
// Función para enviar correo de solicitud de devolución - REMOVED
/*
...
// Add new function to handle marking item as returned
if (isset($_POST['action']) && $_POST['action'] == 'mark_as_returned') {
  header('Content-Type: application/json');
  $user_rfid = $_POST['user_rfid'] ?? null;
  $equipment_rfid = $_POST['equipment_rfid'] ?? null;

  if (!$user_rfid || !$equipment_rfid) {
    echo json_encode(['success' => false, 'message' => 'Faltan datos para procesar la devolución.']);
    exit();
  }

  // Insert a new record with loans_state = 0
  $stmt = $conn->prepare("INSERT INTO `loans` (`loans_hab_rfid`, `loans_equip_rfid`, `loans_state`, `loans_date`) VALUES (?, ?, '0', NOW())");
  if ($stmt === false) {
  ...
  $stmt->bind_param("ss", $user_rfid, $equipment_rfid);
  
  if ($stmt->execute()) {
    echo json_encode(['success' => true, 'message' => 'Equipo marcado como devuelto.']);
  } else {
    error_log("Execute failed: (" . $stmt->errno . ") " . $stmt->error);
    echo json_encode(['success' => false, 'message' => 'Error al marcar como devuelto: ' . $stmt->error]);
  }
...
$(document).ready(function() {
    ...
    // Evento para el botón "Devolver" de cada ítem
    $(document).on('click', '.return-item', function() {
        var userRfid = $(this).data('user-rfid');
        var equipmentRfid = $(this).data('equipment-rfid');
        var equipmentName = $(this).data('equipment-name');

        if (confirm('¿Está seguro que desea marcar el equipo "' + equipmentName + '" como devuelto?')) {
            $.ajax({
                url: 'quitar_prestamo.php',
                method: 'POST',
                data: {
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        // Recargar la lista de préstamos para reflejar el cambio
                        // Usar currentRfid si está seteado (para vista filtrada), sino cargar todos.
                        var rfidToRefresh = currentRfid ? currentRfid : '';
                        $.ajax({
                            url: 'quitar_prestamo.php',
                            method: 'POST',
                            data: { consult_loan: rfidToRefresh },
                            success: function(data) {
                                $('#resultado_').html(""); 
                                var data_ = cortarDespuesDeDoctype(data);
                                $('#resultado_').html(data_);
                                console.log('Refreshed loan data after return.');
                            },
                            error: function() {
                                alert('Error al recargar la lista de préstamos.');
                            }
                        });
                    } else {
                ...
                error: function(xhr, status, error) {
                    alert('Error al procesar la devolución. Por favor, intente de nuevo.');
                    console.error('Error en la solicitud de devolución:', status, error, xhr.responseText);
                }
            });
        }
    });
    ...
    $(document).on('click', '#export-csv', function() {
        ...
        var form = $('<form action="quitar_prestamo.php" method="post" target="_blank"></form>');
